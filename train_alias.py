...
class TrainModel:
    def __init__(self, args):
        ...
        args.update({'semantic_nc': 7}, allow_val_change=True)
        args.update({'alias_layers_D': 10}, allow_val_change=True)
        ...
        args.update({'lambda_fm': 10, 'lambda_percept': 10, 'semantic_nc': 13}, allow_val_change=True)
        ...
        self.optimizer_aliasD = optim.Adam(self.aliasD.parameters(), lr=0.0004, betas=(0, 0.9))
        self.optimizer_aliasD.zero_grad(set_to_none=True)
        
        self.optimizer_aliasG = optim.Adam(self.aliasG.parameters(), lr=0.0001, betas=(0, 0.9))
    ...
    def alias_train_step(self, args, img, img_agnostic, parse, pose, get_img_log=False):
        # Part 3. Try-on synthesis
        with amp.autocast(enabled=args.use_amp):
            cloth_mask_target = parse[:, 3:4]
            cloth_mask_target = cloth_mask_target.permute(0,2,3,1)
            warped_c = ((img+1) * cloth_mask_target) - 1
            warped_cm = parse[:, 3:4]
            misalign_mask = parse[:, 3:4] - warped_cm
            misalign_mask = misalign_mask.clip_(min=0.0)
            parse_div = torch.cat((parse, misalign_mask), dim=1)
            parse_div[:, 3:4] -= misalign_mask #8
            ip = (torch.cat((img_agnostic, pose, warped_c), dim=3))
            ip = ip.permute(0,3,1,2)
            output = self.aliasG(ip, parse, parse_div, misalign_mask)            
        ...
        return alias_lossG.detach_(), alias_lossD.detach_(), loss_G_GAN_Feat, img_log, output
    ...
    def train_epoch(self, args, epoch):
        ...
        with tqdm(self.train_loader.data_loader, desc=f"Epoch {epoch:>2}") as pbar:
            for step, batch in enumerate(pbar):
                ...
                alias_lossG, alias_lossD, loss_G_GAN_Feat, alias_img_log, output = self.alias_train_step(args, img, img_agnostic, parse, pose,
                                                                                 get_img_log=step == (tsteps-2))
                
                                          
                aliasG_losses.update(alias_lossG.detach_(), img_agnostic.size(0))       
                aliasD_losses.update(alias_lossD.detach_(), img_agnostic.size(0)) 
                alias_GAN_Feat_losses.update(loss_G_GAN_Feat, img_agnostic.size(0))                                         
    ...
    def save_models(self, args):
        if args.local_rank == 0:
            ...
            torch.save(self.aliasG.state_dict(), os.path.join(
                args.checkpoint_dir, "aliasG.pth"))
            torch.save(self.aliasD.state_dict(), os.path.join(
                args.checkpoint_dir, "aliasD.pth"))
            print("[+] Weights saved.")
